...
sub OnNavSelectionChanged()
    ...
    print "MAIN ▶ Nav reselect:", m.navBar.reselect

    if m.debugLabel <> invalid
        m.debugLabel.text = "Selected: " + m.navBar.selectedId
    end if

    ' -----------------------------
    ' SAME TAB or DOWN → ENTER SCREEN
    ' -----------------------------
    if m.navBar.reselect = true and m.currentScreen <> invalid
        print "MAIN ▶ Enter current screen"
        m.currentScreen.enterMenu()
        return
    end if

    ' -----------------------------
    ' NEW TAB → NORMAL SWITCH
    ' -----------------------------
